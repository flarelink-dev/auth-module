...
- **Password hashing** — `worker.ts` uses BetterAuth's password verifier (currently PBKDF2-SHA256 since v0.1.1, dropped from scrypt because it was hitting the Workers free-tier CPU budget). Search for `emailAndPassword` in `worker.ts`.
